...
// Global context..now everyone is going to fuck this one..
// But its serial one..not parallel :)
SHA1Context scx;
...
static FORCEINLINE PRUint32 FASTCALL swap4b(PRUint32 dwd) 
{
    __asm 
	{
    	mov   eax,dwd
...
*/
void SHA1_Update(SHA1Context *ctx, const unsigned char *dataIn, unsigned int len) 
{
  ...
  //printf("\n sha1_update , len = %d , lenb = %d , sizeLo= %d , sizeHi = %d ", len,lenB, ctx->sizeLo, ctx->sizeHi);
 ...
 */
void SHA1_End(SHA1Context *ctx, unsigned char *hashout)
{
...
void SHA1Update(const unsigned char *dataIn, unsigned int len)
{
	SHA1_Update(dataIn, len);
}

void SHA1End(unsigned char *hashout, unsigned int *pDigestLen, unsigned int maxDigestLen)
{
	SHA1_End(hashout, pDigestLen, maxDigestLen);
}
*/



SHA1Context *SHA1_GetContext(void)
{
    return &scx;
}
...
{
    return SHA1_HashBuf(dest, (const unsigned char *)src, strlen(src));
}
...
{
    memcpy(space,cx, sizeof(SHA1Context));
    return SECSuccess;
}
...
SHA1Context *SHA1_Resurrect(unsigned char *space,void *arg)
{
    ...
    memcpy(cx,space, sizeof(SHA1Context));
    return cx;
}

void SHA1_TraceState(SHA1Context *ctx)
{
#ifdef TRACING_SSL
    uint32        W;
    int           i;
...
// Here goes our functions...so that they can make use of static functionality
